...
static void
ptr_wise_atomic_list_memmove(PyListObject *a, PyObject **dest, PyObject **src, Py_ssize_t n)
{
...
#define b ((PyListObject *)v)
    ...
    if (d < 0) { /* Delete -d items */
        ...
        ptr_wise_atomic_list_memmove(a, &item[ihigh+d], &item[ihigh], tail);
        ...
        if (list_resize(a, k+d) < 0)
            goto Error;
        item = a->ob_item;
        ptr_wise_atomic_list_memmove(a, &item[ihigh+d], &item[ihigh], k - ihigh);
    }
    for (k = 0; k < n; k++, ilow++) {
        PyObject *w = vitem[k];
...
/*[clinic end generated code: output=6bd69dcb3f17eca8 input=c269141068ae4b8f]*/
{
    ...
    if (index < size_after_pop) {
        ptr_wise_atomic_list_memmove(self, &items[index], &items[index+1],
    ...
    if (_PyIndex_Check(item)) {
        ...
        if (value == NULL) {
            ...
            if (cur < (size_t)Py_SIZE(self)) {
                ptr_wise_atomic_list_memmove(self, self->ob_item + cur - slicelength,
